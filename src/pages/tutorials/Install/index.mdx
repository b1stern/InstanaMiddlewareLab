...
You will see the summary page for Instana.  We'll come back to the user interface later.  Leave the browser open.
...
Before installing the Instana Agent, we need to create a **"Project" (kubernetes namespace)"**" where the Agent will run.  Type the following command to create the project.
```sh
...
If successful, you'll see output similar to what's shown below.
 ...
 Next, change your context to the instana-agent project (kubernetes namespace) by typing "oc project instana-agent".  If you want to confirm that you have changed to the 
 instana-agent project, type:
 ```sh
 oc project
 ```
 Examine the output to make sure the project is **"instana-agent"**


 Next, you need to setup the proper permissions for the Project/Namespace.  This will allow the Agent to have access to monitor the entire cluster.
...
The first thing you need to do is to create namespaces (projects) for the application and the load generation tool.   Then, you need to setup the OpenShift permissions
...
You will be executing the /media/qotd.sh script to execute the helm install. Before you run the shell script, let's take a look at the contents.
```sh
cat /media/qotd.sh
```
...
## Configure Quote of the Day to use and external ACE and MQ server
...
```
This will open a "vi" session.  Scroll down within the file to line 48.  You should see 3 lines of text like the following:
```sh
...
You will modify those linkes to look like the following.  You need to add a "value" parameter that contains the SUPPLY_CHAIN_URL for the ACE flow. The flow within the
...
You will notice that the lab guide within the virtual machine is already open to this set of steps.
...
Save and exist the configuration.yaml file.   In "vi", type ":wq" to save and exit the file.
